...
adts_header::adts_header( Profile profile, uint32_t sample_rate, uint8_t channelCount, int aac_frame_length ) {
    /*
    ...
    */
    ...
    variable_header.aac_frame_length                   = ADTS_HEADER_SIZE + aac_frame_length;
...
uint32_t adts_header::sampleRate( const adts_header &header ) {
    uint32_t sample_rates[] = {
        96000, // 0x0
        88200, // 0x1
        64000, // 0x2
    ...
    return sample_rates[header.fixed_header.sampling_frequency_index];
}

uint8_t adts_header::channelCount( const adts_header &header ) {
    return header.fixed_header.channel_configuration;
}

void adts_header::to_buf( uint8_t buf[7] ) {
    PutBitsContext context = PutBitsContext( buf, 7 );
    // fixed header
    context.put_bits( 12, fixed_header.syncword );
    context.put_bits( 1, fixed_header.id );
    context.put_bits( 2, fixed_header.layer );
    context.put_bits( 1, fixed_header.protection_absent );
    context.put_bits( 2, fixed_header.profile );
    context.put_bits( 4, fixed_header.sampling_frequency_index );
    context.put_bits( 1, fixed_header.private_bit );
    context.put_bits( 3, fixed_header.channel_configuration );
    context.put_bits( 1, fixed_header.original_copy );
    context.put_bits( 1, fixed_header.home );
    // variable header
    context.put_bits( 1, variable_header.copyright_identification_bit );
    context.put_bits( 1, variable_header.copyright_identification_start );
    context.put_bits( 13, variable_header.aac_frame_length );
    context.put_bits( 11, variable_header.adts_buffer_fullness );
    context.put_bits( 2, variable_header.number_of_raw_data_blocks_in_frame );
}
